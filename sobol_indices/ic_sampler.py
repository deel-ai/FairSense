import numpy as np
from math import *
from sobol_indices.sensitivity_indices import sobol_unnormalized, sobol_total_ind_unnormalized, \
    sobol_ind_unnormalized, sobol_total_unnormalized
...
def compute_sobol_table(f, x, y=None, n=1000, N=None):
    ...
    sobol_table = np.vectorize(lambda variable: sobol_indices_at_i(f, variable, mode, n, cov, f_inv), signature='()->(n)')(range(nb_variables))
...
def sobol_indices_at_i(f, variable_index, mode, n, cov, f_inv):
    ...
    if mode == "on_value":
        ...
        zncquad = np.hstack((znc[:, 0], zncbis[:, 1:-1], znc[:, [-1]]))  # possibly wrong !
    ...
    if mode == "on_value":
        ...
        V = np.mean([np.var(f(zc[:, 1:])), np.var(f(zcbis[:, 1:])), np.var(f(zcter[:, 1:])), np.var(f(zcquad[:, 1:]))])
...
def compute_marginal_inv_cumul_dist(data_x: np.ndarray.__class__, N=None):
    ...
    for i in range(data_x.shape[1]):
        ...
        cum_dists_functions.append(np.vectorize(lambda x: x_sorted[min(int(x*len(x_sorted)), len(x_sorted)-1)]))
